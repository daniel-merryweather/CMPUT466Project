...
class QLearningTable:
	def __init__(self, actions, learning_rate = 0.9, reward_decay = 0.9, e_greedy = 0.9, max_states = 10**6):
		...
		try:
			with open ('q_data.pkl', 'rb') as f:
				self.episode, self.q_table = pickle.load(f)
		except:
			print("No previous pickle file")

	def save_output(self):
		with open ('q_data.pkl', "wb") as f:
			pickle.dump([self.episode, self.q_table],f)
...
def sensors_to_index(car, tm):
	...
	index = int(sensors[0] * 10) + int(sensors[2] * 100) + int(sensors[4] * 1000) + int (sensors[6] + 10000) + int( car.vel[1]/(200)*100000)
	if(index < 0):
		index = 0
	return index
